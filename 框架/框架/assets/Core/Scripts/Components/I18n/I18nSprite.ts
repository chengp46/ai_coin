...
export class I18nSpriteData {
    ...
    @property({ displayName: '对应精灵帧', type: SpriteFrame })
...
export class I18nSprite extends I18nBase {
    @property({ displayName: '支持的语言代码列表', type: [I18nSpriteData] })
    ...
    public async refresh(): Promise<void> {
        ...
        const spriteData = this.spList.find((data) => data.langCode === langMgr.language);
        if (spriteData?.spriteFrame) {
            this.sp.spriteFrame = spriteData.spriteFrame;
        } else {
            logMgr.err(`未找到语言代码为 ${langMgr.language} 的精灵帧。`);
        }
    }
}
