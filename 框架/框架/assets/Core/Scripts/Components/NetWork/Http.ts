...
export class Http {
    ...
    public static async post(
        ...
        const { headers, bodyData } = Http.preparePostData(body, contentType, callback);
        if (!headers || !bodyData) {
            logMgr.err('准备POST数据失败，终止请求。');
            return;
        }
     ...
     */
    private static preparePostData(body: any, contentType: string, callback: (data: any) => void): { headers: Record<string, string>, bodyData: string } | null {
        ...
        switch (contentType) {
            ...
            case 'form':
                ...
                break;
            default:
                logMgr.err(`不支持的内容类型: ${contentType}`);
                callback({ error: `不支持的内容类型: ${contentType}` });
                return null;
        }
